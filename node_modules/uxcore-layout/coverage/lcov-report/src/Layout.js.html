...
&nbsp;
  Layout.prototype.getSuffix = function getSuffix(leftAdaptive, rightAdaptive) {
    var suffix = '';
    // 如果都是非自适应，则为 fixed
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!leftAdaptive &amp;&amp; !rightAdaptive) {
      suffix = 'fixed';
    } else <span class="cstat-no" title="statement not covered" >if (!leftAdaptive &amp;&amp; rightAdaptive) {</span>
      // 左侧定宽，右侧自适应，lf
<span class="cstat-no" title="statement not covered" >      suffix = 'lf';</span>
    } else <span class="cstat-no" title="statement not covered" >if (leftAdaptive &amp;&amp; !rightAdaptive) {</span>
      // 左侧自适应，右侧定宽，rf
<span class="cstat-no" title="statement not covered" >      suffix = 'rf';</span>
    } else {
...
&nbsp;
  Layout.prototype.processChildren = function processChildren() {
    ...
    _react2['default'].Children.map(children, function (child) {
      ...
      } else <span class="missing-if-branch" title="else path not taken" >E</span>if (child &amp;&amp; typeof child.type === 'function' &amp;&amp; child.type.displayName === 'Right') {
        me.right = child;
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (me.suffix === 'rf') {
<span class="cstat-no" title="statement not covered" >      me.rightStyle = {</span>
        marginLeft: -me.right.props.width
      };
<span class="cstat-no" title="statement not covered" >      me.leftStyle = {</span>
        paddingRight: me.right.props.width
      };
    } else <span class="missing-if-branch" title="if path not taken" >I</span>if (me.suffix === 'lf') {
<span class="cstat-no" title="statement not covered" >      me.rightStyle = {</span>
...
&nbsp;
  Layout.prototype.renderLayout = function renderLayout(cls) {
    var me = this;
    var arr = [];
    <span class="missing-if-branch" title="if path not taken" >I</span>if (me.suffix === 'lf') {
<span class="cstat-no" title="statement not covered" >      arr.push(me.renderRight(cls));</span>
<span class="cstat-no" title="statement not covered" >      arr.push(me.renderLeft(cls));</span>
    } else {
...
<div class='footer quiet pad2 space-top1 center small'>
  Code coverage
  generated by <a href="http://istanbul-js.org/" target="_blank">istanbul</a> at Wed Sep 12 2018 20:31:10 GMT+0800 (CST)
</div>
</div>
